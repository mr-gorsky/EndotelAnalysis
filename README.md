...
2. **Kalibracija (µm/piksel):**
   - Ne postoji javno objavljena specifikacija (rezolucija senzora / vidno
     ...
     - **🔧 Kalibracija ravnalom**: snimi mjernu pločicu/mm papir kroz isti
       ...
       - **Linija (2 točke)** — klikneš prvu pa drugu točku poznate
         udaljenosti, upiši tu udaljenost u mm.
       - **Pravokutnik / mreža** — **povučeš pravokutnik i rastegneš ga
         mišem** (uhvatiš kut ili rub i vučeš) dok mu rubovi točno ne legnu
         na poznati pravokutnik na slici (npr. kvadratić milimetarskog
         papira) — isti princip kao razvlačenje mreže u Phoenixu.
...
3. **✂️ Crop**: **povučeš pravokutnik preko slike i rastegneš ga mišem**
   (uhvatiš kut ili rub i vučeš) da odabereš dio za analizu. Kalibracija
   (µm/px) ostaje ista prije i poslije cropa, jer crop samo bira piksele,
   ne mijenja njihovu veličinu.

   I crop i pravokutna kalibracija koriste `streamlit-cropper` — pravi
   pravokutnik s ručkama za vučenje/rastezanje (isto ponašanje kao alati za
   rezanje slike u drugim programima), ne ručni unos brojeva niti niz
   pojedinačnih klikova.
